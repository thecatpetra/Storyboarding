...
module RealisticSnow =
    let flakeLoop ts te image =
        let (fx, fy) = SbRandom.choice [(SbRandom.randX(), 0); (744, SbRandom.randY())]
        ...
        >>= movey ts te (fy + SbRandom.randInt 0 100) (fy + 480 + SbRandom.randInt 0 100) >> easing Easing.SineInOut
        >>= movex ts te (fx) (fx - 844))

    let effect ts te =
        let image = gp_filled_circle |> resize1To 32 |> pad 6 |> gaussBlur 4f
        printfn $"Making snow ({ts}/{te})"
        monadicMap [0..64] (fun i -> flakeLoop ts te image)